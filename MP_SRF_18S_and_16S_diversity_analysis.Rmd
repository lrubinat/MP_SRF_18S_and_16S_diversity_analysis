---
title: "MP_SRF_amplicons_18S_and_16S_diversity_analysis"
author: "lrubinat"
date: "05/03/2016"
output:
  html_document:
...
tb18_tax_occur_ss8522_no_cero_t.rabund_means.occurence_perc<-as.data.frame(100*(rowSums(tb18_tax_occur_ss8522_no_cero_t.rabund.occur)/117))
...
Dinophyceae_tb_occur <- Dinophyceae_tb[,1:117]
...
Prasinophyceae_tb_occur <- Prasinophyceae_tb[,1:117]
...
Chrysophyceae_tb_occur <- Chrysophyceae_tb[,1:117]
...
Pelagophyceae_tb_occur <- Pelagophyceae_tb[,1:117]
...
Dictyochophyceae_tb_occur <- Dictyochophyceae_tb[,1:117]
...
Cryptomonadales_tb_occur <- Cryptomonadales_tb[,1:117]
...
Bacillariophyta_tb_occur <- Bacillariophyta_tb[,1:117]
...
Chlorarachniophyta_tb_occur <- Chlorarachniophyta_tb[,1:117]
...
Bolidophyceae_tb_occur <- Bolidophyceae_tb[,1:117]
...
Pinguiochysidales_tb_occur <- Pinguiochysidales_tb[,1:117]
...
Prymnesiophyceae_tb_occur <- Prymnesiophyceae_tb[,1:117]
...
Mamiellophyceae_tb_occur <- Mamiellophyceae_tb[,1:117]
...
Eustigmatales_tb_occur <- Eustigmatales_tb[,1:117]
...
Chlorophyceae_tb_occur <- Chlorophyceae_tb[,1:117]
...
Ulvophyceae_tb_occur <- Ulvophyceae_tb[,1:117]
...
Raphydophyceae_tb_occur <- Raphydophyceae_tb[,1:117]
...
Trebouxiophyceae_tb_occur <- Trebouxiophyceae_tb[,1:117]
...
class_summary_reads_per_class<-aggregate(rowSums(tb18_phototrophs[1:117]), list(tb18_phototrophs$MAS_plus_BM_plus_SILVA_class), sum)
# count the different groups

class_summary_otus_per_class<-aggregate(rowSums(tb18_phototrophs[1:117]), list(tb18_phototrophs$MAS_plus_BM_plus_SILVA_class), length)
...
tb18S_OTUs_reads_samples$samples_per_class<-c(117,117,117,111,116,95,90,97,95,89,47,32,27,25,11,19,6)
...
otus_vs_samples_plot + geom_point() + geom_text(check_overlap = TRUE,hjust = -0.05, nudge_x = 0.05, size=3) + scale_x_log10(limits = c(-1, 15000), breaks=c(0.1,1,10,100,1000,1000)) + scale_y_continuous(trans = "log10") + labs(title="[2] Reads per class vs. OTUs per class", x="OTUs (log10)", y="reads (log10)") 
```
...
tb18S_OTUs_reads_samples_rel_abund$samples_per_class<-(tb18S_OTUs_reads_samples_rel_abund$samples_per_class*100)/117
...
ggplot(tb18S_OTUs_reads_samples_rel_abund) +
  geom_point(aes(samples_per_class,OTUs_per_class)) +
  geom_text_repel(aes(samples_per_class,OTUs_per_class, label = rownames(tb18S_OTUs_reads_samples)), size=4.5, force = 5, box.padding = unit(0.1, "lines"), point.padding = unit(0.2, 'lines')) + scale_x_continuous(limits = c(0, 116), breaks=c(25,50,75,100)) + labs(title="[3.2] OTUs per class vs. samples in which they occur", x="\nMP-iTags-18S samples (%)", y="MP-iTags-18S OTUs (%)\n") + coord_trans(y = "log10") + theme(axis.text=element_text(size=14), axis.title=element_text(size=16))
```
...
ggplot(tb18S_OTUs_reads_samples_rel_abund) +
  ...
  geom_text_repel(aes(OTUs_per_class, reads_per_class, label = rownames(tb18S_OTUs_reads_samples)), size=3, force = 3, box.padding = unit(0.10, "lines"), point.padding = unit(0.2, 'lines')) + labs(title="[4.2] Reads per class vs. OTUs per class", x="MP-iTags-18S OTUs (%)", y="MP-iTags-18S reads (%)") + coord_trans(y = "log10") + scale_x_log10(limits = c(-1,200), breaks = c(0.1,1,10,100))
...
tb18_tax_occur_min8522_t<-tb18_tax_occur_min8522
tb18_min8522_tax<-merge(tb18_tax_occur_min8522_t,tb18_class, by="row.names")

dim(tb18_min8522_tax)
tb18_min8522_tax[1:5,1:5]

#fix OTU_no as new row
rownames(tb18_min8522_tax)=tb18_min8522_tax$Row.names

#add OTU_no as rowname
rownames.tb18_min8522_tax<-tb18_min8522_tax[,1]
tb18_min8522_tax<-tb18_min8522_tax[,-1]
#colnames(tb18_min8522_tax, do.NULL=F)

dim(tb18_min8522_tax)
tb18_min8522_tax[1:5, 1:5]

#sort by OTU_no (split rowname, introduce no. into column "OTU_no" and sort)
tb18_min8522_tax["OTU_no"] <- NA
tb18_min8522_tax$OTU_no <- sapply(strsplit(rownames(tb18_min8522_tax),split= "\\_"),'[',2)
tb18_min8522_tax$OTU_no <- as.numeric(as.character(tb18_min8522_tax$OTU_no))
tb18_min8522_tax_sorted<-tb18_min8522_tax[order(tb18_min8522_tax$OTU_no, decreasing = FALSE), ]

dim(tb18_min8522_tax_sorted)
tb18_min8522_tax_sorted[1:5,1:5]
```

```{r select_phototrophs_non.norm_18S, echo=FALSE}
tb18_phototrophs_non.norm <- tb18_min8522_tax_sorted[which(tb18_min8522_tax_sorted$MAS_plus_BM_plus_SILVA_class != "" & tb18_min8522_tax_sorted$MAS_plus_BM_plus_SILVA_class != "NA"),]
...
class_summary_reads_per_class_non.norm<-aggregate(rowSums(tb18_phototrophs_non.norm[1:117]), list(tb18_phototrophs_non.norm$MAS_plus_BM_plus_SILVA_class), sum)
# count the different groups

class_summary_otus_per_class_non.norm<-aggregate(rowSums(tb18_phototrophs_non.norm[1:117]), list(tb18_phototrophs_non.norm$MAS_plus_BM_plus_SILVA_class), length)
...
tb18S_OTUs_reads_samples_non.norm$samples_per_class<-c(117,117,117,113,116,103,112,112,108,111,97,65,35,75,75,50,18)
...
tb18S_OTUs_reads_samples_rel_abund_non.norm$samples_per_class<-(tb18S_OTUs_reads_samples_rel_abund_non.norm$samples_per_class*100)/117
...
otus_vs_samples_plot <- ggplot(tb18S_OTUs_reads_samples_rel_abund_non.norm, aes(samples_per_class,OTUs_per_class, label = rownames(tb18S_OTUs_reads_samples)))
otus_vs_samples_plot + geom_point() + geom_text(check_overlap = TRUE,hjust = -0.05, nudge_y = 0.05, size = 3) + scale_x_continuous(limits = c(0, 116), breaks=c(25,50,75,100)) + labs(title="[5] OTUs per class vs. samples in which they occur", x="MP-iTags-18S samples (%)", y="OTUs (%)") + coord_trans(y = "log10")
```

Reads per class vs. OTUs per class [PLOT DESCRIPTION]

```{r OTUs_vs_reads_non.norm_18S, echo=FALSE}
otus_vs_samples_plot <- ggplot(tb18S_OTUs_reads_samples_rel_abund_non.norm, aes(OTUs_per_class, reads_per_class, label = rownames(tb18S_OTUs_reads_samples)))
...
#read data 
tb16_tax <- read.table(file="/home/laura/Documents/TFM/home/data/MALASPINA/Malaspina_16S_Surface/table_with_BLAST/MP_16S_SRF_tax_classif_tab_filtered_v2.txt", head=TRUE, fill=TRUE)

#table dimensions and format before setting column names
dim(tb16_tax) # 960 129
tb16_tax[1:5,1:5]
...
Datasets summary:
dim(tb16_tax) --> 993 128
dim(tb16_tax_occur) --> 960 122
dim(tb16_tax_occur_ss5836_no_cero) --> 117 909
...
tb16_tax_occur_ss5836_div <- diversity(tb16_tax_occur_ss5836_no_cero, index="shannon")
```

Most of the samples take Shannon Index values between 5.5 and 6:

```{r shannon_index2_16S, echo=FALSE}
boxplot(tb16_tax_occur_ss5836_div, pch=19, main="Shannon's index of diversity")
plot(sort(tb16_tax_occur_ss5836_div), pch=19, main="Shannon's index of diversity")
```

### 2.3.2) Richness: OTU number

```{r richness_otu_no1_16S, echo=FALSE}
OTUs_per_sample_16S_tax_occur_ss5836<-specnumber(tb16_tax_occur_ss5836_no_cero)
```

Lowest number of OTUs per sample:

```{r richness_otu_no2_16S, echo=FALSE}
min(OTUs_per_sample_16S_tax_occur_ss5836)
```

Maximum number of OTUs per sample:

```{r richness_otu_no3_16S, echo=FALSE}
max(OTUs_per_sample_16S_tax_occur_ss5836)
```
...
tb16_tax_occur_min5836_radfit<-radfit(colSums(tb16_tax_occur_min5836_t))
plot(tb16_tax_occur_min5836_radfit)
```
...
tb16_tax_occur_ss5836_dist_all<-prestondistr(colSums(tb16_tax_occur_min5836_t))
plot(tb16_tax_occur_ss5836_prestonfit, main="All malaspina")
lines(tb16_tax_occur_ss5836_dist_all, line.col="blue3")

veiledspec(tb16_tax_occur_ss5836_dist_all)
```
...
#?vegdist
tb16_tax_occur_ss5836_no_cero.bray<-vegdist(tb16_tax_occur_ss5836_no_cero, method="bray")
boxplot(tb16_tax_occur_ss5836_no_cero.bray, main="Bray-Curtis dissimilarity matrix")
```
...
#UPGMA
tb16_tax_occur_ss5836_no_cero.upgma<-hclust(tb16_tax_occur_ss5836_no_cero.bray, "average")
plot(tb16_tax_occur_ss5836_no_cero.upgma, cex=.35, main="Samples Hierarchical Clustering")
```
...
#NMDS
tb16_tax_occur_ss5836_no_cero.nmds<-monoMDS(tb16_tax_occur_ss5836_no_cero.bray)
tb16_tax_occur_ss5836_no_cero.nmds
plot(tb16_tax_occur_ss5836_no_cero.nmds, main="monoMDs method")
```

When implementing a most robut function for computing NMDS plots, the result is quiet the same:

```{r metaNMDS_16S, echo=F}
tb16_tax_occur_ss5836_no_cero.meta_nmds<-metaMDS(tb16_tax_occur_ss5836_no_cero.bray)
plot(tb16_tax_occur_ss5836_no_cero.meta_nmds, main="metaMDS method")
```
...
MP_geo_16S_sorted<-MP_geo_16S[row.names(tb16_tax_occur_ss5836_no_cero),]
...
1) Community matrix: tb16_tax_occur_ss5836_no_cero
...
2) Community Bray-Curtis: tb16_tax_occur_ss5836_no_cero.bray

```{r working_datasets2_16S, echo=2}
dim(tb16_tax_occur_ss5836_no_cero.bray)
...
```{r mantel_correlogram1_18S_vs_16S, echo=T}
...
colSums(tb16_tax_occur_ss5836_no_cero_t)

#local abundance percentage
tb16_tax_occur_ss5836_no_cero_t.rabund<-tb16_tax_occur_ss5836_no_cero_t/5836

colSums(tb16_tax_occur_ss5836_no_cero_t.rabund)
tb16_tax_occur_ss5836_no_cero_t.rabund[1:5,1:5]

#OTUs mean relative abundance
tb16_tax_occur_ss5836_no_cero_t.rabund_means<-rowMeans(tb16_tax_occur_ss5836_no_cero_t.rabund) 
tb16_tax_occur_ss5836_no_cero_t.rabund_means<-as.data.frame(tb16_tax_occur_ss5836_no_cero_t.rabund_means)

head(tb16_tax_occur_ss5836_no_cero_t.rabund_means)
```

```{r OTUs_occurence_16S, echo=F, results='hide'}
tb16_tax_occur_ss5836_no_cero_t.rabund.occur<-tb16_tax_occur_ss5836_no_cero_t.rabund
tb16_tax_occur_ss5836_no_cero_t.rabund.occur[tb16_tax_occur_ss5836_no_cero_t.rabund.occur>0]<-1
tb16_tax_occur_ss5836_no_cero_t.rabund.occur[1:5,1:5] ### presence - absence table

#percentage of occurence in overall stations
tb16_tax_occur_ss5836_no_cero_t.rabund_means.occurence_perc<-as.data.frame(100*(rowSums(tb16_tax_occur_ss5836_no_cero_t.rabund.occur)/117))

str(tb16_tax_occur_ss5836_no_cero_t.rabund_means.occurence_perc)
```

```{r merge_rabund_peroccur_16S, echo=F, results='hide'}
otu_tb16_ss5836_rabund_percoccur<-merge(tb16_tax_occur_ss5836_no_cero_t.rabund_means,tb16_tax_occur_ss5836_no_cero_t.rabund_means.occurence_perc, by="row.names")

colnames(otu_tb16_ss5836_rabund_percoccur)<-c("OTUs","mean_rabund","perc_occur")
otu_tb16_ss5836_rabund_percoccur[1:5,]

row.names(otu_tb16_ss5836_rabund_percoccur)<-otu_tb16_ss5836_rabund_percoccur[,1]
otu_tb16_ss5836_rabund_percoccur<-otu_tb16_ss5836_rabund_percoccur[,-1]
otu_tb16_ss5836_rabund_percoccur[1:5,]
```
...
plot(otu_tb16_ss5836_rabund_percoccur$mean_rabund,otu_tb16_ss5836_rabund_percoccur$perc_occur, log="x", pch=19, cex=0.8, xlab="Mean relative abundance", ylab="Percentage of occurence")
abline(h=80, col="red") #occurence higher than 80%
abline(v=0.00001, col="green") #rare OTUs
abline(v=0.001, col="blue") #cosmopolitan OTUs
...
#regionally abundant
tb16_ss5836_abundant<-otu_tb16_ss5836_rabund_percoccur[otu_tb16_ss5836_rabund_percoccur$mean_rabund > 0.001,]

tb16_ss5836_abundant_sorted<-tb16_ss5836_abundant[order(tb16_ss5836_abundant$mean_rabund, tb16_ss5836_abundant$perc_occur, decreasing = T), c(1,2)]

tb16_ss5836_abundant_sorted
...
#length(row.names(otu_tb16_ss5836_rabund_percoccur[otu_tb16_ss5836_rabund_percoccur$mean_rabund > 0.001,])) # 83 OTUs
#row.names(otu_tb16_ss5836_rabund_percoccur[otu_tb16_ss5836_rabund_percoccur$mean_rabund > 0.001,])
```

Cosmopolitan OTUs (relative abundance over 0.1% and occurence in more than 80% of samples):

```{r select_cosmopolitan_16S, echo=6}
otu_tb16_ss5836_rabund_cosm<-otu_tb16_ss5836_rabund_percoccur[otu_tb16_ss5836_rabund_percoccur$mean_rabund > 0.001,]
otu_tb16_ss5836_rabund_poccur_cosm<-otu_tb16_ss5836_rabund_cosm[otu_tb16_ss5836_rabund_cosm$perc_occur > 80,]
otu_tb16_ss5836_cosmop_sorted<-otu_tb16_ss5836_rabund_poccur_cosm[order(otu_tb16_ss5836_rabund_poccur_cosm$perc_occur, otu_tb16_ss5836_rabund_poccur_cosm$mean_rabund, decreasing = T), c(1,2)]

otu_tb16_ss5836_cosmop_sorted
...
dim(otu_tb16_ss5836_rabund_percoccur)
dim(tb16_tax_occur_min5836)

#rare OTUs:
# length(row.names(otu_tb16_ss5836_rabund_percoccur[otu_tb16_ss5836_rabund_percoccur$mean_rabund < 0.00001 & otu_tb16_ss5836_rabund_percoccur$mean_rabund >0 ,])) # 325 OTUs
```
--->
...
dim(tb16_protists)
dim(tb16_bacteria)
tb16_protists[1:5,117:124]
tb16_bacteria[1:5,117:124]
```
...
other_plastids_tb_occur <- other_plastids_tb[,1:117]
...
Rappemonad_tb_occur <- Rappemonad_tb[,1:117]
...
heterotrophic_bacteria_tb_occur <- heterotrophic_bacteria_tb[,1:117]
...
Cyanobacteria_tb_occur <- Cyanobacteria_tb[,1:117]
...
tb16S_protists_OTUs_reads_samples$samples_per_class<-c(117,117,114,98,31,26,21,27,13,13,2)
...
tb16S_bacteria_OTUs_reads_samples$samples_per_class<-c(117,117)
...
otus_vs_samples_plot_protists + geom_point() + geom_text(check_overlap = TRUE,hjust = -0.05, nudge_y = 0.05, size = 3) + scale_x_continuous(limits = c(0, 137)) + labs(title="[7] OTUs per class vs. samples in which they occur", x="samples", y="OTUs") + coord_trans(y = "log10")
```
...
otus_vs_samples_plot_protists + geom_point() + geom_text(check_overlap = TRUE,hjust = -0.05, nudge_x = 0.05, size=3) + scale_x_continuous(limits = c(0, 32)) + labs(title="[8] Reads per class vs. OTUs per class", x="OTUs", y="reads") + coord_trans(y = "log10")
```
...
otus_vs_samples_plot_protists_plus_bacteria + geom_point() + geom_text(check_overlap = TRUE,hjust = -0.05, nudge_y = 0.05, size = 3) + scale_x_continuous(limits = c(0, 137)) + labs(title="[9] OTUs per class vs. samples in which they occur", x="samples", y="OTUs")
```
...
otus_vs_samples_plot_protists_plus_bacteria + geom_point() + geom_text(check_overlap = TRUE,hjust = -0.05, nudge_x = 0.05, size=3) + scale_x_continuous(limits = c(0, 35)) + labs(title="[10] Reads per class vs. OTUs per class", x="OTUs", y="reads") + coord_trans(y = "log10")
```
...
tb16S_protists_OTUs_reads_samples_rel_abund$samples_per_class<-(tb16S_protists_OTUs_reads_samples_rel_abund$samples_per_class*100)/117
...
otus_vs_samples_plot_protists + geom_point() + geom_text(check_overlap = TRUE,hjust = -0.05, nudge_x = 0.05, size=3) + scale_x_continuous(limits = c(0, 58)) + labs(title="[12] Reads per class vs. OTUs per class", x="OTUs (%)", y="reads (%)") + coord_trans(y = "log10")
```
...
tb16S_protists_plus_bacteria_OTUs_reads_samples_rel_abund$samples_per_class<-(tb16S_protists_plus_bacteria_OTUs_reads_samples_rel_abund$samples_per_class*100)/117
...
otus_vs_samples_plot_protists_plus_bacteria + geom_point() + geom_text(check_overlap = TRUE,hjust = -0.05, nudge_y = 0.05, size = 3) + scale_x_continuous(limits = c(0, 120)) + labs(title="[13] OTUs per class vs. samples in which they occur", x="MP-iTags-16S samples (%)", y="MP-iTags-16S OTUs (%)") + coord_trans(y = "log10")

ggplot(tb16S_protists_plus_bacteria_OTUs_reads_samples_rel_abund) +
  geom_point(aes(samples_per_class,OTUs_per_class)) +
  geom_text_repel(aes(samples_per_class,OTUs_per_class, label = rownames(tb16S_protists_plus_bacteria_OTUs_reads_samples_rel_abund)), size=3, force = 5, box.padding = unit(0.10, "lines"), point.padding = unit(0.2, 'lines'), nudge_y = 0.5) + scale_x_continuous(limits = c(0, 120)) + labs(title="[13.2] OTUs per class vs. samples in which they occur", x="MP-iTags-16S samples (%)", y="MP-iTags-16S OTUs (%)") + coord_trans(y = "log10")
...
otus_vs_samples_plot_protists_plus_bacteria + geom_point() + geom_text(check_overlap = TRUE,hjust = -0.05, nudge_x = 0.05, size=3) + scale_x_continuous(limits = c(0, 40)) + labs(title="[14] Reads per class vs. OTUs per class", x="OTUs (%)", y="reads (%)") + coord_trans(y = "log10") 


ggplot(tb16S_protists_plus_bacteria_OTUs_reads_samples_rel_abund) +
  geom_point(aes(OTUs_per_class, reads_per_class)) +
  geom_text_repel(aes(OTUs_per_class, reads_per_class, label = rownames(tb16S_protists_plus_bacteria_OTUs_reads_samples_rel_abund)), size=3, force = 0.5, box.padding = unit(0.10, "lines"), point.padding = unit(0.2, 'lines'))  + scale_x_continuous(limits = c(0, 40)) + labs(title="[14.2] Reads per class vs. OTUs per class", x="MP-iTags-16S OTUs (%)", y="MP-iTags-16S reads (%)") + coord_trans(y = "log10")
```

### 2.6.2) Non-normalized data
...
tb16_protists_non.norm[1:5,117:124]
tb16_bacteria_non.norm[1:5,117:124]
```
...
tb16S_protists_OTUs_reads_samples_non.norm$samples_per_class<-c(117,117,114,98,31,26,21,27,13,13,2)
...
tb16S_bacteria_OTUs_reads_samples_non.norm$samples_per_class<-c(117,117)
...
tb16S_protists_OTUs_reads_samples_non.norm_rel_abund$samples_per_class<-(tb16S_protists_OTUs_reads_samples_non.norm_rel_abund$samples_per_class*100)/117
...
otus_vs_samples_plot_protists + geom_point() + geom_text(check_overlap = TRUE,hjust = -0.05, nudge_x = 0.05, size=3) + scale_x_continuous(limits = c(0, 58)) + labs(title="[16] Reads per class vs. OTUs per class", x="OTUs (%)", y="reads (%)") + coord_trans(y = "log10")
```
...
tb16S_protists_plus_bacteria_OTUs_reads_samples_non.norm_rel_abund$samples_per_class<-(tb16S_protists_plus_bacteria_OTUs_reads_samples_non.norm_rel_abund$samples_per_class*100)/117
