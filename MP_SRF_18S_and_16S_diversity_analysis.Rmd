...
tb18S_OTUs_reads_sample <- merge(class_summary_reads_per_class_order, class_summary_otus_per_class_order, by="row.names")

row.names(tb18S_OTUs_reads_sample)<-tb18S_OTUs_reads_sample[,1]
tb18S_OTUs_reads_sample<-tb18S_OTUs_reads_sample[,-1]
colnames(tb18S_OTUs_reads_sample)<-c("reads_per_class","OTUs_per_class")
tb18S_OTUs_reads_sample[1:5,1:2]

#sort
# SORT tb18S_OTUs_reads_sample<-tb18_ss8522_abundant[order(tb18_ss8522_abundant$mean_rabund, tb18_ss8522_abundant$perc_occur, decreasing = T), c(1,2)]

#add no. of samples per class
tb18S_OTUs_reads_sample["samples_per_class"] <- NA
tb18S_OTUs_reads_sample$samples_per_class<-c()
```




```{r OTUs_vs_reads_plot, echo=FALSE}
png("otus_vs_reads.png")

plot(class_summary_otus_per_class_order, class_summary_reads_per_class_order, pch=19, cex=0.4, xlab="Number of OTUs", ylab="Number of reads", main = "#OTUs vs #reads")

with(class_summary_otus_per_class_order[1:17,], text(x~x,labels = row.names(class_summary_otus_per_class_order[1:17,]), pos = 4))
...
```{r merge_tables_2_16S, echo=FALSE}
tb16_tax_occur_ss5844_no_cero_t<-t(tb16_tax_occur_ss5844_no_cero)
...
```{r select_phototrophs_2_16S, echo=FALSE}
